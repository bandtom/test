...
get_latest_ver(){
    ...
    v2_file=$(wget -qO- https://api.github.com/repos/teddysun/v2ray-plugin/releases/latest | grep linux-amd64 | grep name | cut -f4 -d\")
}
...
install_v2(){
    if [ -f /usr/local/bin/v2ray-plugin ];then
        ...
        if [ ! -f $v2_file ];then
            v2_url=$(wget -qO- https://api.github.com/repos/teddysun/v2ray-plugin/releases/latest | grep linux-amd64 | grep browser_download_url | cut -f4 -d\")
            wget $v2_url
        fi
        tar xf $v2_file
